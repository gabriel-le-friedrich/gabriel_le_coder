...
/*
// Apply user data to current chapter
function applyUserDataToChapter() {
    if (!state.user) return;
    
    // Clear any existing user data indicators first
    document.querySelectorAll('.bookmark-icon, .note-icon').forEach(icon => {
        icon.remove();
    });
    
    // Reset verse background colors
    ...
    highlights.forEach(highlight => {
        const verseElement = document.querySelector(`.verse[data-verse-id*="_${highlight.verse}"]`);
        if (verseElement) {
    ...
    bookmarks.forEach(bookmark => {
        const verseElement = document.querySelector(`.verse[data-verse-id*="_${bookmark.verse}"]`);
        if (verseElement) {
            const bookmarkIcon = document.createElement('i');
            bookmarkIcon.className = 'fas fa-bookmark bookmark-icon';
            bookmarkIcon.title = 'Bookmarked verse';
            
            // Insert at the beginning of the verse
            if (verseElement.firstChild) {
                verseElement.insertBefore(bookmarkIcon, verseElement.firstChild);
            } else {
    ...
    notes.forEach(note => {
        const verseElement = document.querySelector(`.verse[data-verse-id*="_${note.verse}"]`);
        if (verseElement) {
            const noteIcon = document.createElement('i');
            noteIcon.className = 'fas fa-sticky-note note-icon';
            noteIcon.title = note.text.length > 30 ? note.text.substring(0, 30) + '...' : note.text;
            
            // Insert after bookmark icon if it exists, otherwise at the beginning
            const bookmarkIcon = verseElement.querySelector('.bookmark-icon');
            if (bookmarkIcon) {
                verseElement.insertBefore(noteIcon, bookmarkIcon.nextSibling);
            } else if (verseElement.firstChild) {
                verseElement.insertBefore(noteIcon, verseElement.firstChild);
            } else {
                verseElement.appendChild(noteIcon);
            }
            
            // Add click handler to open note
            noteIcon.addEventListener('click', (e) => {
                e.stopPropagation(); // Prevent verse click action
                showNoteEditor(state.currentBook, state.currentChapter, note.verse);
            });
        }
    });
}
